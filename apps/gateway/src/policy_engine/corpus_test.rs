...
// resolves relative to this file (apps/gateway/src/policy_engine/), so four
// `..` reach the repo root. Coupling the gateway test build (this module is
...
fn corpus_engine_matches_expected() {
    ...
    for case in cases {
        ...
        let body = case.request.body.as_deref().map(str::as_bytes);
...
fn directory_identity_rule_matches_via_principal_set() {
    ...
    let v2_row = |identities: serde_json::Value, is_default: bool| crate::db::PolicyRuleV2Row {
        id: "r".to_string(),
    ...
    let rules = super::assemble::assemble_v2(
        &org,
        &[],
        &crate::db::SecretHosts::default(),
        &crate::db::ConnectionProviders::default(),
    );
    ...
    assert_eq!(
        evaluate_new(&rules, &req(vec!["g1".to_string()]), None),
        Decision::block(),
...
fn empty_target_non_default_rule_matches_nothing() {
    ...
    let row =
        |targets: serde_json::Value, is_default: bool, action: &str| crate::db::PolicyRuleV2Row {
            id: "r".to_string(),
            logical_id: "lr".to_string(),
            name: "rule".to_string(),
            source: "custom".to_string(),
    ...
    assert_eq!(
        evaluate_new(&orphaned, &request, None),
        Decision::allow(),
    ...
    assert_eq!(
        evaluate_new(&with_target, &request, None),
        Decision::block(),
...
fn secret_target_permits_its_host() {
    ...
    let row =
        |action: &str, is_default: bool, targets: serde_json::Value| crate::db::PolicyRuleV2Row {
            id: "r".to_string(),
            logical_id: "lr".to_string(),
            name: "rule".to_string(),
            source: "custom".to_string(),
    ...
    let secret_hosts = crate::db::SecretHosts {
        by_id: HashMap::from([
    ...
    let decide = |org: &[crate::db::PolicyRuleV2Row],
                  workspace: &[crate::db::PolicyRuleV2Row],
                  host: &str| {
        let rules = super::assemble::assemble_v2(
            org,
            workspace,
        ...
        evaluate_new(&rules, &req(host), None)
    };
...
fn app_scope_and_connection_targets_permit_their_provider_hosts() {
    ...
    let providers = crate::db::ConnectionProviders {
        by_id: HashMap::from([
    ...
    let decide_req = |org: &[crate::db::PolicyRuleV2Row],
                      workspace: &[crate::db::PolicyRuleV2Row],
        ...
        evaluate_new(&rules, request, None)
    };
    let decide = |org: &[crate::db::PolicyRuleV2Row],
                  workspace: &[crate::db::PolicyRuleV2Row],
                  host: &str| {
...
fn law_row(action: &str, is_default: bool, path: Option<&str>) -> crate::db::PolicyRuleV2Row {
    use sqlx::types::Json;
...
fn workspace_default_allow_is_neutral() {
    let check = |org: &[crate::db::PolicyRuleV2Row],
                 without: &[crate::db::PolicyRuleV2Row],
                 with: &[crate::db::PolicyRuleV2Row],
                 label: &str| {
...
fn outcome_attributes_the_winning_rule() {
    ...
    match evaluate_outcome(&rules, &floor_req("/api", true, false), None) {
        Outcome::Rule(rule) => {
